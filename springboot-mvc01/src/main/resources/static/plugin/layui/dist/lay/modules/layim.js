...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  var listTpl = function(options){
    ...
    return ['{{# var length = 0; layui.each('+ options.item +', function(i, data){ length++; }}'
      ,'<li layim-event="chat" data-type="'+ options.type +'" data-index="{{ '+ (options.index||'i') +' }}" class="layim-'+ (options.type === 'history' ? '{{i}}' : options.type + '{{data.id}}') +' {{ data.status === "offline" ? "layim-list-gray" : "" }}"><img src="'+jsTool.getProjectName()+'/static/common/images/userHeadPortrait.png"  draggable="false" class="" style="display: none;" data-imgurl="{{ data.avatar }}" data-class="" data-style="" data-title="" data-alt="" onload="tool.getPictureLoading(this)"><span>{{ data.username||data.groupname||data.name||"佚名" }}</span><p>{{ data.remark||data.sign||"" }}</p><span class="layim-msg-status">new</span></li>'
    ,'{{# }); if(length === 0){ }}'
  ...
  var elemAddTpl = ['<div class="layim-add-box">'
    ,'<div class="layim-add-img"><img src="'+jsTool.getProjectName()+'/static/common/images/userHeadPortrait.png"   class="layui-circle" style="" data-imgurl="{{ d.data.avatar }}" data-class="layui-circle" data-style="" data-title="" data-alt="" onload="tool.getPictureLoading(this)"><p>{{ d.data.name||"" }}</p></div>'
  ...
  var elemChatMain = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }} {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }}>'
    ,'<div class="layim-chat-user"><img src="'+jsTool.getProjectName()+'/static/common/images/userHeadPortrait.png"   class="" style="" data-imgurl="{{ d.avatar }}" data-class="" data-style="" data-title="" data-alt="" onload="tool.getPictureLoading(this)"><cite>'
    ,'{{# if(d.mine){ }}'
  ...
  var elemChatList = '<li class="layim-{{ d.data.type }}{{ d.data.id }} layim-chatlist-{{ d.data.type }}{{ d.data.id }} layim-this" layim-event="tabChat"><img src="'+jsTool.getProjectName()+'/static/common/images/userHeadPortrait.png"   class="" style="" data-imgurl="{{ d.data.avatar }}" data-class="" data-style="" data-title="" data-alt="" onload="tool.getPictureLoading(this)"><span>{{ d.data.name||"佚名" }}</span>{{# if(!d.base.brief){ }}<i class="layui-icon" layim-event="closeChat">&#x1007;</i>{{# } }}</li>';
  ...
  var layimClose, popmin = function(content){
    ...
    return layer.open({
      ...
      ,offset: 'rb'
      ,resize: false
      ,content: '<img src="'+jsTool.getProjectName()+'/static/common/images/userHeadPortrait.png"   class="" style="" data-imgurl="'+ (cache.mine.avatar||(layui.cache.dir+'css/pc/layim/skin/logo.jpg')) +'" data-class="" data-style="" data-title="" data-alt="" onload="tool.getPictureLoading(this)"><span>'+ (content||cache.base.title||'我的聊天') +'</span>'
  ...
  var setChatMin = function(newMsg){
    ...
    setChatMin.index = layer.open({
      ...
      ,resize: false
      ,area: ['182px', '50px']
      ,content: '<img src="'+jsTool.getProjectName()+'/static/common/images/userHeadPortrait.png" id="layui-layim-min"  class="" style="" data-imgurl="'+ thatChat.avatar +'" data-id="layui-layim-min" data-class="" data-style="" data-title="" data-alt="" onload="tool.getPictureLoading(this)"><span>'+ thatChat.name +'</span>'
      ,success: function(layero, index){
  ...
  var anim = 'layui-anim-upbit', events = {
        ...
        if(val === ''){
          ...
          if(data.length > 0){
            for(var l = 0; l < data.length; l++){
              html += '<li layim-event="chat" data-type="'+ data[l].type +'" data-index="'+ data[l].index +'" data-list="'+ data[l].list +'"><img src="'+jsTool.getProjectName()+'/static/common/images/userHeadPortrait.png"   class="" style="" data-imgurl="'+ data[l].avatar +'" data-class="" data-style="" data-title="" data-alt="" onload="tool.getPictureLoading(this)"><span>'+ (data[l].username || data[l].groupname || '佚名') +'</span><p>'+ (data[l].remark||data[l].sign||'') +'</p></li>';
            }
          } else {
    ...
    ,find: function(){
      ...
      return events.find.index = jp.openViewDialog("管理界面",cache.base.find,"800px", "500px");
    }
    ...
    //关于
    ,about: function(){
      layer.alert('版本： '+ v + '<br>版权所有：'+tool.getCopyright(), {
        title: '关于'
        ,shade: false
      });
    }
          ...
          ,success: function(layero){
            ...
            post(members, function(res){
              layui.each(res.list, function(index, item){
                li += '<li data-uid="'+ item.id +'"><a href="javascript:;"><img src="'+jsTool.getProjectName()+'/static/common/images/userHeadPortrait.png"   class="" style="" data-imgurl="'+ item.avatar +'" data-class="" data-style="" data-title="" data-alt="" onload="tool.getPictureLoading(this)"><cite>'+ item.username +'</cite></a></li>';
